    ...
    useEffect(() => {
        if (preselectedFloor && preselectedBuilding) {
            ...
            const filteredFloors = floors.filter(
                floor => floor.building && floor.building.id === preselectedBuilding.id
            );
    ...
    useEffect(() => {
        ...
        if (effectiveFloorId) {
            ...
            if (floor && floor.building) {
                ...
                console.log("Available floors IDs:", floors.map(f => f.id));
            }
        } else {
            console.log("No floor ID provided, user is creating a room from scratch");
        }
    }, [floors, selectedFloorId, window.location.search]);

    // Additional fallback - check localStorage
    useEffect(() => {
        if (!selectedFloorId && !preselectedFloor && typeof localStorage !== 'undefined') {
            const storedFloorId = localStorage.getItem('lastSelectedFloorId');
            if (storedFloorId) {
                console.log("Fallback: Found floor ID in localStorage:", storedFloorId);
                
                // Find the floor
                const floor = floors.find(f => f.id === parseInt(storedFloorId, 10));
                if (floor && floor.building) {
                    console.log("Fallback: Found floor and building from localStorage");
                    setComingFromFloor(true);
                    setSelectedBuilding(floor.building.id.toString());
                    ...
                    const filteredFloors = floors.filter(
                        f => f.building && f.building.id === floor.building.id
                    );
                    setBuildingFloors(filteredFloors);
                }
            }
                                ...
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2 mt-4">
                                    <div>
                                        ...
                                        {comingFromFloor && (
                                            <Text className="mt-1 text-xs text-gray-500">
                                                Building is pre-selected based on the floor you came from
                                            </Text>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            disabled={!selectedBuilding || comingFromFloor}
                                            required
                                        >
                                        ...
                                        {comingFromFloor && (
                                            <Text className="mt-1 text-xs text-gray-500">
                                                Floor is pre-selected based on where you came from
                                            </Text>
                                        )}
                                    </div>
                                </div>
